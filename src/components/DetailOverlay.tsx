  ...
  return (
    <div className="overlay-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section ref={dialogRef} className="detail-overlay" role="dialog" aria-modal="true" aria-label={`${show.title} details`}>
        <div className="detail-art" style={{ backgroundImage: `url(${show.image})` }}>
          ...
          <div className="detail-heading">
            ...
            <div className="detail-actions">
              <button className="play-button" onClick={onPlay}><Play weight="fill" /> Play</button>
              <button className={`icon-button ${listed ? 'is-active' : ''}`} onClick={onList} aria-label={listed ? 'Remove from watchlist' : 'Add to watchlist'} title="Watchlist">
                {listed ? <Check weight="bold" /> : <Plus weight="bold" />}
              </button>
              <button className={`icon-button ${liked ? 'is-active' : ''}`} onClick={onLike} aria-label={liked ? 'Unlike' : 'Like'} title="Like">
